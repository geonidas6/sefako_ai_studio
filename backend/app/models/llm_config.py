...
class LLMConfig(Base):
    ...
    One row per provider (gemini, anthropic, openai, openrouter, nvidia, huggingface, grok, groq, mistral, qwen).
    Also stores agent-to-provider assignments as rows with provider='assign_<agent>'.
    """
    __tablename__ = "llm_configs"

    id: Mapped[str] = mapped_column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
    # provider key: gemini | anthropic | openai | openrouter | nvidia | huggingface | grok | groq | mistral | qwen | assign_strategy | assign_ux | ...
